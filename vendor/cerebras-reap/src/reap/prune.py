...
def prune(
    ...
    for layer in tqdm(observer_data, "Pruning layers..."):
        ...
        if not model_attrs["fused"]:
            ...
            # Preserve custom ModuleList subclasses (e.g. MiniMaxM2Experts) that
            # implement a specialized forward(hidden, top_k_index, top_k_weights).
            experts_cls = type(all_experts)
            if experts_cls is torch.nn.ModuleList:
                retained_module = torch.nn.ModuleList(retained_experts)
            else:
                retained_module = experts_cls.__new__(experts_cls)
                torch.nn.ModuleList.__init__(retained_module, retained_experts)
                for attr_name in ("top_k", "hidden_dim", "ffn_dim", "act_fn"):
                    if hasattr(all_experts, attr_name):
                        setattr(retained_module, attr_name, getattr(all_experts, attr_name))
                if hasattr(all_experts, "num_experts"):
                    retained_module.num_experts = len(retained_experts)
            setattr(moe, model_attrs["experts"], retained_module)
            if hasattr(moe, "num_experts"):
                moe.num_experts = len(retained_experts)
            ...
            # MiniMax / some MoE blocks keep correction bias on the MoE module itself
            if hasattr(moe, "e_score_correction_bias"):
                moe.e_score_correction_bias.data = moe.e_score_correction_bias.data[
                    retained_expert_indicies
                ]

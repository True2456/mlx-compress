...
class MoETransformerObserverConfig(BaseTransformerObserverHookConfig):
    ...
    # "softmax" (default) or "sigmoid" (MiniMax / DeepSeek-style gated MoE)
    router_score_fn: str = "softmax"
    # When True, add moe.e_score_correction_bias before top-k (MiniMax / GLM-style)
        ...
        def _hook_fn(module, args, output):
            ...
            if self.hook_config.fused_experts:
                ...
                *_, router_logits = output  # (total_tokens, num_experts)
                if getattr(self.hook_config, "use_score_correction_bias", False) and hasattr(
                    module, "e_score_correction_bias"
                ):
                    scores = torch.sigmoid(router_logits.float())
                    scores_for_choice = scores + module.e_score_correction_bias
            ...
            pruning_batch = update_pruning_state(
                ...
                router_score_fn=getattr(self.hook_config, "router_score_fn", "softmax"),
            )

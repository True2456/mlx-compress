...
def update_pruning_state(
    ...
    *,
    ...
    if router_score_fn == "sigmoid":
        routing_weights = torch.sigmoid(pruning_batch.router_logits.float()).to(device)
    elif router_score_fn == "softmax":
        routing_weights = F.softmax(
            pruning_batch.router_logits, dim=1, dtype=torch.float
        ).to(device)
    else:
        raise ValueError(
            f"Unknown router_score_fn={router_score_fn!r}; expected 'softmax' or 'sigmoid'"
        )
